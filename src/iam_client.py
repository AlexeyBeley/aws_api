...
class IamClient(Boto3Client):
    ...
    def get_all_users(self, full_information=True):
        ...
        for response in self.execute("list_users", "Users"):
            user = IamUser(response)
            final_result.append(user)

        if full_information:
            response = self.execute("get_account_authorization_details", "UserDetailList")
    ...
    def update_policy_statements(self, policy):
        ...
        response = self.execute("get_policy_version", "PolicyVersion", filters_req={"PolicyArn": policy.arn, "VersionId": policy.default_version_id})
    ...
    def get_all_policies(self, full_inforamtion=True):
        ...
        for result in self.execute("list_policies", "Policies"):
